...
from data_utils import load_one_graph
...
class MFInstSegDataset(BaseDataset):
    ...
    def __init__(self, 
        ...
        # load data partition from train.txt valid.txt test.txt file
        if split == "all":
            print('path', os.path.join('MFInstseg_partition', 'train.txt'))
            ...
            print('split_filelist',os.path.join('E:/TGNet/MFInstseg_partition', split+'.txt'))
            with open(os.path.join('E:/TGNet/MFInstseg_partition', split+'.txt'), 'r') as f: #"E:\TGNet\MFInstseg_Partition"
    ...
    def load_one_graph(self, fn, data):
        ...
        assert num_faces == len(seg_label), \
            'File {} have wrong number of labels {} with faces {}. '.format(
...
if __name__ == '__main__':
    ...
    tempDir = 'D:\\dataset\\databj-526' #dataall-2025410
    #dataset = MFInstSegDataset(root_dir=tempDir, split='test', center_and_scale=True, normalize=False)

    filelist = _get_filenames(tempDir)
    # 70% for train, 15% for valid, 15% for test
    train_split = int(0.7 * len(filelist))
    valid_split = int(0.15 * len(filelist))
    num_train_data = -1
    train_filelist = filelist[:train_split][:num_train_data]
    writeDataset('E:\\TGNet\\MFInstseg_partition\\train.txt',train_filelist)

    val_filelist = filelist[train_split:train_split+valid_split]
    writeDataset('E:\\TGNet\\MFInstseg_partition\\val.txt', val_filelist)

    test_filelist = filelist[train_split+valid_split:]
    writeDataset('E:\\TGNet\\MFInstseg_partition\\test.txt', test_filelist)
